...
namespace Exosuit
{
    public class JobDriver_GetInWalkerCore_Drafted : JobDriver_GetInWalkerCore
    {
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            Toil gearUp = new()
            {
                initAction = () =>
                {
                    if (pawn.CurJob.GetTarget(TargetIndex.A).Thing is Building_MaintenanceBay bay)
                    {
                        pawn.Position = bay.Position;
                        bay.GearUp(pawn);
    ...
    public class JobDriver_GetInWalkerCore : JobDriver
    {
        ...
        public override bool TryMakePreToilReservations(bool errorOnFailed)
        {
            ...
            if (Bay.Core.Extesnsion is ExosuitExt mod and not null)
            {

...
namespace Exosuit
{
    public class Building_EjectorBay : Building, IThingHolder
    {
            ...
            if (this.HasPawn)
            {
                Command_Target command_Eject = new()
                {
                    ...
                    action = delegate (LocalTargetInfo target)
                    {
                        ...
                        WG_AbilityVerb_QuickJump.DoJump(
                            p, destMap, target, new LocalTargetInfo(
                                new IntVec3(
                                    p.Position.x, p.Position.y, Mathf.Min(p.Position.z + 25, destMap.Size.z))), false, false);
                    }
                };
        ...
        {
            if (!pawn.IsColonist && !pawn.IsSlaveOfColony && !pawn.IsPrisonerOfColony && (!pawn.IsColonySubhumanPlayerControlled || !pawn.IsGhoul))
            {
                return false;
            }
            ...
            if (!pawn.apparel.WornApparel.Any((a) => a is Exosuit_Core)) return "CannotEnterBuilding".Translate(this) + ": " + "WG_RequireMechsuit".Translate().CapitalizeFirst();
        ...
        {
            base.ExposeData();
            Scribe_Deep.Look(ref innerContainer, "Container",
            [
                this
            ]);

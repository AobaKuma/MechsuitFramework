...
namespace Exosuit
{
    ...
    public static class ModuleUtil
    {
        ...
        static ModuleUtil()
        {
            Modules = DefDatabase<ThingDef>.AllDefs.Where((ThingDef def) => def.category == ThingCategory.Item && !def.IsApparel && def.HasComp<CompSuitModule>()).ToArray();
            Slots = DefDatabase<SlotDef>.AllDefs.ToArray();
        }
        public static List<ThingDef> SlotUsers(SlotDef slot)
        {
            return Modules.Where(m => m.GetCompProperties<CompProperties_ExosuitModule>().occupiedSlots.Contains(slot)).ToList();
        }
    }
